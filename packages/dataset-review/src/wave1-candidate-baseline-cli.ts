...
import { buildWave1CandidateBaseline } from "./wave1-candidate-baseline.js";
...
async function main(): Promise<void> {
  ...
  const report = buildWave1CandidateBaseline({
    bundle,
...
main().catch((error: unknown) => {
  ...
  process.stderr.write(`wave1-candidate-baseline failed: ${message}\n`);
  process.exitCode = 1;
});
