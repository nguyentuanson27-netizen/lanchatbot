...
const PosPriceEvidenceSchema = z.object({
  ...
  if (Date.parse(value.expiresAt) - Date.parse(value.observedAt) !==
    PRICE_INVENTORY_FRESH_FOR_SECONDS * 1_000) {
    context.addIssue({
      code: z.ZodIssueCode.custom,
      path: ["expiresAt"],
      message: "POS price evidence must use the 48-hour freshness window",
    });
  }
});
...
function blocked(
  reasonCode: Extract<ShopPolicyEvaluation, { status: "BLOCKED" }>['reasonCode'],
): ShopPolicyEvaluation {
...
export function deriveClosingCustomerState(rawEvidence: unknown): ClosingCustomerState | null {
  ...
  if (evidence.priceObjectionCount >= 2 && evidence.secondConcessionAlreadyApplied) {
    return evidence.previousState === "HESITANT" || evidence.previousState === "CAUTIOUS"
      ? "CAUTIOUS"
      : null;
  }
  if (evidence.priceObjectionCount >= 1 && !evidence.secondConcessionAlreadyApplied) {
    return evidence.previousState === null ||
      evidence.previousState === "READY" ||
      evidence.previousState === "HESITANT"
      ? "HESITANT"
      : null;
  }
  return evidence.previousState === null || evidence.previousState === "READY"
    ? "READY"
    : null;
}
...
export function evaluateShopPolicy(input: ShopPolicyEvaluationInput): ShopPolicyEvaluation {
  ...
  if (new Set(lines.map(({ lineId }) => lineId)).size !== lines.length) {
    return blocked("DUPLICATE_LINE_ID");
  }
  ...
  for (const line of lines) {
    ...
    if (![nextCount, lineTotal, nextSubtotal].every(Number.isSafeInteger)) {
      return blocked("CART_TOTAL_UNSAFE");
    }
  ...
  if (parentProductUnitCount >= policy.multiItemOffer.minimumProductCount) {
    adjustments.push({
  ...
  if (customerState === "HESITANT") {
    ...
    adjustments.push(
      {
        ruleId: "SHOP_WIDE_FINAL_FREE_SHIPPING",
        kind: "FREE_SHIPPING",
        amountVnd: shippingFeeVnd,
        percentageBps: null,
      },
      {
        ruleId: "SHOP_WIDE_FINAL_FIXED_DISCOUNT",
        kind: "FIXED_DISCOUNT",
        amountVnd: policy.negotiation.finalConcession.fixedDiscountVnd,
        percentageBps: null,
      },
    );
  }

  const discountTotalVnd = adjustments.reduce((sum, adjustment) =>
    sum + adjustment.amountVnd, 0);
  return {
    status: "AUTHORIZED",
