...
describe("commerce kernel architecture boundary", () => {
  ...
  it("uses the authorization clock for policy and POS freshness while preserving artifact time", () => {
    const metadata = {
      authority: "ADMIN_POLICY" as const,
      sourceVersion: "policy-v1",
      observedAt: "2026-08-14T00:00:00.000Z",
    ...
    const policy = PolicyBundleV1Schema.parse({
      ...
      closing: {
        customerStates: ["READY", "HESITANT", "CAUTIOUS"],
        decisionMode: "DETERMINISTIC_POLICY_ONLY", allowUnlistedOffers: false, metadata,
      },
    });
    const result = evaluateShopPolicy({
      policy,
      lines: [{
        lineId: "line-1", shopId: "LANA", parentProductId: "P-001",
        offerKind: "DIRECT", quantity: 1,
        priceEvidence: {
          authority: "PANCAKE_POS", shopId: "LANA", parentProductId: "P-001",
          offerKind: "DIRECT", sourceVersion: "pos-v1",
          observedAt: "2026-08-14T00:00:00.000Z",
          expiresAt: "2026-08-16T00:00:00.000Z", freshForSeconds: 172_800,
          unitPriceVnd: 100_000,
        },
      }],
      closingEvidence: {
        previousState: null, purchaseReady: true, priceObjectionCount: 0,
        secondConcessionAlreadyApplied: false,
      },
      now: new Date("2026-08-14T00:20:00.000Z"),
      authorizationNow: new Date("2026-08-14T01:00:00.000Z"),
    });

    expect(result).toMatchObject({ status: "BLOCKED", reasonCode: "POLICY_EXPIRED" });
  });
});
