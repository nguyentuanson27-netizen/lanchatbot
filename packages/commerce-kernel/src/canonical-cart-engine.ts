...
export type CanonicalCartCreationV2 =
  ...
  | {
      readonly status: "BLOCKED";
      readonly reasonCode: "CART_SEED_INVALID" | "POLICY_BLOCKED" | "PRICE_FACT_INVALID";
      readonly policyReasonCode: string | null;
    };
...
function mutate(
  lines: readonly CartLineV1[],
  mutation: CanonicalCartMutationV2 | null,
): readonly CartLineV1[] | null {
  ...
  if (mutation.kind === "SET_QUANTITY") {
    if (!Number.isInteger(mutation.quantity) || mutation.quantity < 1 || mutation.quantity > 20) {
      return null;
    }
  ...
  const color = mutation.color === null
    ? null
    : mutation.color.trim().normalize("NFC").toUpperCase();
...
export function canonicalCartPolicyLinesV2(
  lines: readonly CartLineV1[],
  shopId: string,
): readonly PolicyCartLineInput[] | null {
  ...
  for (const line of lines) {
    ...
    if (
      ...
      authority.shopId !== shopId ||
      authority.parentProductId !== line.parentProductId ||
      authority.offerId !== line.offerId
    ) return null;
    result.push({
      lineId: line.lineId,
      shopId,
...
function evaluatedCart(input: {
  ...
  try {
    ...
    return {
      ...
      policyReasonCode: "POLICY_INPUT_INVALID",
    };
  }
  ...
  const adjustments = evaluation.adjustments.map((adjustment) => ({
    adjustmentId: deterministicUuid(
      `adjustment\u0000${input.base.cartId}\u0000${input.nextVersion}\u0000${adjustment.ruleId}`,
    ),
    ...
    policyAuthorization: {
      ...
      decisionId: deterministicUuid(
        `decision\u0000${input.base.cartId}\u0000${input.nextVersion}\u0000${adjustment.ruleId}`,
      ),
...
/** Creates revision 1 from authoritative POS-backed lines. */
export function createCanonicalCartV2(input: {
  ...
  if (parsedLines.some(({ success }) => !success)) {
    return { status: "BLOCKED", reasonCode: "CART_SEED_INVALID", policyReasonCode: null };
  }
  ...
  if (!("schemaVersion" in evaluated)) {
    return {
      status: "BLOCKED",
      reasonCode: evaluated.reasonCode === "PRICE_FACT_INVALID"
        ? "PRICE_FACT_INVALID"
        : "POLICY_BLOCKED",
      policyReasonCode: evaluated.policyReasonCode,
    };
  }
  return { status: "CREATED", cart: evaluated };
}

/** Canonical cart reducer shared by the writer and transaction verifier. */
export function applyCanonicalCartDecisionV2(input: {
  ...
  const blocked = (
    reasonCode: Extract<CanonicalCartDecisionV2, { status: "BLOCKED" }>['reasonCode'],
    policyReasonCode: string | null = null,
  ): CanonicalCartDecisionV2 => ({ status: "BLOCKED", cart, reasonCode, policyReasonCode });
  if (cart.revision !== input.expectedCartVersion) return blocked("CART_VERSION_CONFLICT");
  if (cart.status === "CONFIRMED" || cart.status === "ABANDONED") {
    return blocked("CART_TERMINAL");
  }
  if (!Number.isFinite(input.now.getTime()) || input.shopId.trim() === "") {
  ...
  if (
    input.readyForConfirmation === true &&
    validLines.some((line) => line.components.some((component) => component.size === null))
  ) return blocked("CART_MUTATION_INVALID");
  const nextVersion = cart.revision + 1;
  const next = evaluatedCart({
    base: cart,
    lines: validLines,
    nextVersion,
    shopId: input.shopId,
