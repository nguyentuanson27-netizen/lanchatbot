-- Pending Track B B3.2 V2-to-last-known-good-V2 fence artifact. Applying it
-- requires separate owner authorization, exact ENGINEERING_PREPROD
-- backup/rehearsal and checksum readback. It replaces only the 0037 insert
-- guard; it never moves a behavior pointer or releases a fence.
...
BEGIN
  ...
  IF to_regclass('schema_migrations') IS NULL
     ...
     OR NOT EXISTS (
       SELECT 1 FROM schema_migrations
        WHERE migration_name = '0038_track_b_commerce_admission_gate'
          AND checksum_sha256 = '9dcf65e97671777991ad366cdb738ee986b4ee943635a744884c8733f4001140'
     )
    ...
    RAISE EXCEPTION '0039 requires exact applied migrations 0036, 0037 and 0038';
  END IF;
  ...
  IF replacement_guard_hash IS DISTINCT FROM
    ...
    RAISE EXCEPTION '0039 requires exact 0037 replacement guard identity';
  END IF;
  IF EXISTS (
    SELECT 1
    ...
    RAISE EXCEPTION '0039 up requires zero unreleased cutover fences';
  END IF;
END;
$$;
...
BEGIN
  ...
  allowed_transition :=
    ...
    (NEW.page_id = '1198992073286645'
      AND NEW.channel = 'MESSENGER'
      AND pre_version.confirmation_mode = 'V2_ACTIVE'
      AND pre_version.sales_authority_mode = 'COMMERCE'
      AND pre_version.authority_bundle_hash = v2_bundle
      AND target_version.authority_bundle_hash = v2_bundle
      AND NEW.pre_cutover_version_id = NEW.target_version_id
      AND NEW.pre_cutover_content_hash = NEW.target_content_hash);
...
COMMENT ON FUNCTION guard_df13_commerce_cutover_fence_insert_identity() IS
  'Exact 0036 first cutover, historical V1-to-V2 replacement, and page-scoped V2-to-same-identity-V2 LKG service cutover guard; never moves a pointer or permits V1 rollback.';
