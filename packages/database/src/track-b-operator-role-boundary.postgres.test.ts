...
postgresDescribe("0040 Track B operator PostgreSQL boundary", () => {
  ...
  it("rejects an expired-lease token reuse and invalidates the old token on fresh reacquire", async () => {
    ...
    await pool.query("INSERT INTO runtime_behavior_mode_pointers(page_id,channel,active_version_id,pointer_revision,updated_by,reason,updated_at) VALUES($1,'MESSENGER',$2,1,'fixture','fixture',now()) ON CONFLICT (page_id,channel) DO NOTHING", [page, versionId]);
    const revision = Number((await pool.query("SELECT pointer_revision FROM runtime_behavior_mode_pointers WHERE page_id=$1 AND channel='MESSENGER'", [page])).rows[0]?.pointer_revision);
    const canonical = `{"channel":"MESSENGER","operationId":"${operationId}","pageId":"${page}","preCutover":{"contentHash":"${contentHash}","modeVersionId":"${versionId}","pointerRevision":${revision}},"schemaVersion":1,"target":{"authorityBundleHash":"${bundle}","contentHash":"${contentHash}","modeVersionId":"${versionId}"}}`;
    const fingerprint = createHash("sha256").update(canonical).digest("hex");
    const oldHash = "d".repeat(64); const newHash = "e".repeat(64);
    const first = await operator.query("SELECT * FROM track_b_operator_acquire_fence(gen_random_uuid(),$1,$2,$3,$4,$5,$6,$7,60000)", [operationId, versionId, contentHash, revision, bundle, fingerprint, oldHash]);
    ...
    await expect(operator.query("SELECT * FROM track_b_operator_acquire_fence(gen_random_uuid(),$1,$2,$3,$4,$5,$6,$7,60000)", [operationId, versionId, contentHash, revision, bundle, fingerprint, oldHash])).rejects.toThrow(/TOKEN_REUSE/u);
    const renewed = await operator.query("SELECT * FROM track_b_operator_acquire_fence(gen_random_uuid(),$1,$2,$3,$4,$5,$6,$7,60000)", [operationId, versionId, contentHash, revision, bundle, fingerprint, newHash]);
