...
SELECT
  ...
  catalog_version,
  occurred_at,
  CASE WHEN event_type LIKE 'WAVE2_%'
    THEN event_metadata #>> '{wave2Strategy,need}' ELSE NULL END AS wave2_need,
  CASE WHEN event_type LIKE 'WAVE2_%'
    THEN event_metadata #>> '{wave2Strategy,barrier}' ELSE NULL END AS wave2_barrier,
  CASE WHEN event_type LIKE 'WAVE2_%'
    THEN event_metadata #>> '{wave2Strategy,decisionFactor}' ELSE NULL END AS wave2_decision_factor,
  CASE WHEN event_type LIKE 'WAVE2_%'
    THEN event_metadata #>> '{wave2Strategy,recommendedStrategy}' ELSE NULL END AS wave2_strategy,
  CASE WHEN event_type LIKE 'WAVE2_%'
    THEN event_metadata #>> '{wave2Strategy,ctaPolicy}' ELSE NULL END AS wave2_cta_policy,
  CASE WHEN event_type LIKE 'WAVE2_%'
    THEN event_metadata #>> '{wave2Strategy,experimentId}' ELSE NULL END AS wave2_experiment_id,
  CASE WHEN event_type LIKE 'WAVE2_%'
    THEN event_metadata #>> '{wave2Strategy,experimentVariant}' ELSE NULL END AS wave2_experiment_variant,
  CASE
    WHEN event_metadata #>> '{decisionObservability,schemaVersion}' = '1' THEN 1
    ELSE NULL
  END AS decision_observability_schema_version,
  CASE
    WHEN event_metadata #>> '{decisionObservability,schemaVersion}' = '1'
      AND jsonb_typeof(event_metadata #> '{decisionObservability,dialogueEvidence,codes}') = 'array'
      THEN COALESCE(
        (
  ...
  CASE
    WHEN event_metadata #>> '{decisionObservability,schemaVersion}' = '1'
      AND event_metadata #>> '{decisionObservability,buyingIntent,decision}' IN (
        'NONE', 'CONSIDERING', 'COMMITTED', 'NEGATED'
      )
  ...
  CASE
    WHEN event_metadata #>> '{decisionObservability,schemaVersion}' = '1'
      AND event_metadata #>> '{decisionObservability,protectedClaimValidation,outcome}' IN (
        'NOT_EVALUATED', 'NO_PROTECTED_CLAIMS', 'VALIDATED', 'PARTIALLY_BLOCKED', 'BLOCKED'
      )
  ...
  CASE
    WHEN event_metadata #>> '{decisionObservability,schemaVersion}' = '1'
      AND event_metadata #>> '{decisionObservability,readiness,outcome}' IN (
        'NOT_EVALUATED', 'LEGACY_READY', 'LEGACY_NOT_READY'
      )
  ...
  CASE
    WHEN event_metadata #>> '{decisionObservability,schemaVersion}' = '1'
      AND event_metadata #>> '{decisionObservability,phaseBarrier,phase}' IN (
        'NOT_EVALUATED', 'DISCOVERY', 'PRODUCT_MATCHED', 'FACTS_PRESENTED',
        'FIT_CONSULTING', 'MEASUREMENTS_REQUIRED', 'SIZE_RECOMMENDED',
        'OBJECTION_HANDLING', 'READY_TO_BUY', 'CART_OPEN', 'ORDER_REVIEW',
        'ORDER_PREVIEW', 'PURCHASE_CONFIRMED', 'HANDED_OFF', 'POST_SALE'
      )
  ...
  CASE
    WHEN event_metadata #>> '{decisionObservability,schemaVersion}' = '1'
      AND event_metadata #>> '{decisionObservability,phaseBarrier,barrier}' IN (
        'NOT_EVALUATED', 'NONE', 'BARRIER_PRICE', 'BARRIER_FIT',
        'BARRIER_MATERIAL', 'BARRIER_TRUST', 'BARRIER_DELIVERY', 'CHOICE_OVERLOAD'
      )
  ...
  CASE
    WHEN event_metadata #>> '{decisionObservability,schemaVersion}' = '1'
      AND event_metadata #>> '{decisionObservability,strategyCta,strategy}' IN (
        'NONE', 'STRATEGY_RECOMMEND_PRODUCT', 'STRATEGY_SHOW_PROOF',
        'STRATEGY_ANSWER_OBJECTION', 'STRATEGY_ASK_CLARIFY', 'STRATEGY_CLOSE'
      )
  ...
  CASE
    WHEN event_metadata #>> '{decisionObservability,schemaVersion}' = '1'
      AND event_metadata #>> '{decisionObservability,strategyCta,cta}' IN (
        'NONE', 'ASK_OCCASION', 'ASK_STYLE', 'ASK_BUDGET', 'ASK_MEASUREMENTS',
        'ASK_PROOF_PREFERENCE', 'REDUCE_TO_TWO', 'POST_MEDIA_CLOSE', 'NO_ADDITIONAL_CTA'
      )
  ...
  CASE
    WHEN event_metadata #>> '{decisionObservability,schemaVersion}' = '1'
      AND event_metadata #>> '{decisionObservability,reconciliation,outcome}' IN (
        'NOT_APPLIED', 'PRESERVED', 'OVERRIDDEN', 'REJECTED'
      )
  ...
  CASE
    WHEN event_metadata #>> '{decisionObservability,schemaVersion}' = '1'
      AND event_metadata #>> '{decisionObservability,sideEffectPlan,disposition}' IN (
        'NONE', 'PLANNED', 'SAFE_FALLBACK_PLANNED', 'BLOCKED'
      )
