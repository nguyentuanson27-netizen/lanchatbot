/*
 * Authored-journey fixture contract.
 *
 * Kept separate from the CLI loader so the same rules that gate
 * `journeys.json` can be exercised directly with negative fixtures. The
 * simulation materializer builds canonical evidence from these fixtures
 * instead of re-parsing them through the canonical schemas, so this contract
 * is the effective boundary for authored journey state.
 */
const ok = (value, message) => { if (!value) throw new Error(message); };

export const JOURNEY_THEMES = Object.freeze([
...
/**
 * Product-scoped evidence must name a bound product. An empty binding
 * (UNRESOLVED / NOT_REQUIRED) binds no product, so it admits no
 * product-scoped evidence at all: nothing downstream re-checks claim or fact
 * scope against the binding before the model sees it.
 */
function validateEvidenceScope(journeyId, turnId, context, deps) {
  const bound = new Set(context.product_binding.product_ids);
  for (const ref of context.runtime_claim_refs) {
    const claim = deps.runtimeClaims[ref];
  ...
  for (const ref of context.simulation_fact_refs) {
    const fact = deps.simulationFacts[ref];
    if (fact.productId) {
...
function validatePhaseProjection(journeyId, turnId, context, deps) {
  const projection = deps.materialization.context_projection;
...
/**
 * Mirrors AgentBuyingIntentV1Schema (packages/contracts) exactly, in the
 * fixture's snake_case shape: NONE carries no action, quantity or evidence;
 * any other decision requires exact evidence; only COMMITTED may carry a cart
 * action or quantity, and COMMITTED requires one. Agreement with the canonical
 * schema is pinned by track-c-c2-journey-contract.test.ts.
 */
function validateBuyingIntent(journeyId, turnId, intent) {
  const at = `${journeyId}/${turnId}`;
  ok(intent !== null && typeof intent === "object" && !Array.isArray(intent),
    `${at}: buying intent`);
  ok(JSON.stringify(Object.keys(intent).sort()) ===
    JSON.stringify(["decision", "evidence", "quantity", "requested_action"]),
  `${at}: buying intent fields`);
  ok(decisions.has(intent.decision), `${at}: buying decision`);
  ok(requestedActions.has(intent.requested_action), `${at}: requested action`);
  ok(intent.quantity === null ||
    (Number.isInteger(intent.quantity) && intent.quantity >= 1 && intent.quantity <= 20),
  `${at}: buying quantity`);
  ok(intent.evidence === null ||
    (typeof intent.evidence === "string" &&
      intent.evidence.trim().length >= 1 &&
      intent.evidence.trim().length <= 1_000),
  `${at}: buying evidence`);

  if (intent.decision === "NONE") {
    ok(intent.requested_action === "NONE" &&
      intent.quantity === null &&
      intent.evidence === null,
    `${at}: NONE buying intent cannot contain action, quantity or evidence`);
    return;
  }
  ok(intent.evidence !== null, `${at}: non-NONE buying intent requires exact evidence`);
  ok(intent.decision === "COMMITTED" ||
    (intent.requested_action === "NONE" && intent.quantity === null),
  `${at}: only COMMITTED buying intent may request a cart action or quantity`);
  ok(intent.decision !== "COMMITTED" || intent.requested_action !== "NONE",
    `${at}: COMMITTED buying intent requires a requested action`);
}
 ...
 */
function validateNextMove(journeyId, turnId, nextMove, allowedActions) {
...
/**
 * Validates a supplemental journey bundle. `deps` carries the C2-owned
 * catalogs the fixtures reference: runtimeClaims, simulationFacts,
 * materialization, and the corpus-owned allowedActions vocabulary.
 */
export function validateJourneyBundle(bundle, deps) {
  const themes = new Set(JOURNEY_THEMES);
  const runtimeRefs = new Set(Object.keys(deps.runtimeClaims));
  const simulationRefs = new Set(Object.keys(deps.simulationFacts));
  ...
  ok(bundle.evidence_class === "MANUAL_REVIEW_ONLY",
    "journeys must declare manual-review evidence class");
  ...
  for (const journey of bundle.journeys) {
    ...
    ok(!hasImplementationKey(journey),
      `${journey.id}: candidate implementation detail leaked into fixture`);
    ...
    for (const [index, turn] of journey.turns.entries()) {
      ...
      validateEvidenceScope(journey.id, turn.id, context, deps);
      validateBuyingIntent(journey.id, turn.id, context.buying_intent);
      validatePhaseProjection(journey.id, turn.id, context, deps);
      validateCheckout(
        journey.id,
        turn.id,
      ...
      validateNextMove(journey.id, turn.id, expected.next_move, deps.allowedActions);
    }
  }
