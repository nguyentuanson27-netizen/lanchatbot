...
`benchmark_revision` is **R2.9**. The 70 DEV case IDs and rubric remain fixed. Four fresh cart cases now carry a frozen shipping-fee snapshot. The offline materializer constructs one canonical cart and policy binding, then derives the selected claim from that same binding; the C3 candidate requires the readback before it can state a cart fact. Q024 remains blocked by the missing negative-freeship producer, and Q027 remains an expired capture. Q092 uses the reachable open-cart stage and declares its missing checkout fields; Q100 declares checkout complete. Production-contract reachability remains separately classified and does not gain authority from these simulation inputs.
