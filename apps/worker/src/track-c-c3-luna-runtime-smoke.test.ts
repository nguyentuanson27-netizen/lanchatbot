...
describe.skipIf(!enabled)("Track C Luna RealtimeRunner smoke (opt in)", () => {
  it("runs 9 synthetic stateful journeys through RealtimeRunner and saves full artifacts", async () => {
    ...
    for (let journeyIndex = 0; journeyIndex < journeys.length; journeyIndex += 1) {
      ...
      for (let turnIndex = 0; turnIndex < journey.turns.length; turnIndex += 1) {
        ...
        if (reply) history.appendBot(conversationId, reply, at);
        turns.push({ sequence, customerText: entry.envelope.message.text, processed,
          ...
          selectedCartSize: selectedSize, reply });
    ...
    const sizeEdit = records[7] as { turns: readonly { selectedCartSize: string; stateAfter: { commerce: unknown } }[]; finalCommerceState: unknown };
    ...
    expect((records[7] as { turns: readonly unknown[] }).turns).toHaveLength(7);
    expect((records[8] as { turns: readonly unknown[] }).turns).toHaveLength(8);
    expect(modelCalls.length).toBeGreaterThanOrEqual(32);
  }, 60 * 60 * 1000);
});
