...
export const TRACK_C_C3_RESPONDER_SYSTEM_INSTRUCTION = [
  ...
  "Otherwise, if checkoutCompleteness state is REQUIRED: ask only for its missingFields exactly once; use CLARIFICATION target CHECKOUT_DETAILS and ACTION_REQUEST PROVIDE_CHECKOUT_DETAILS as one objective, strategy ASK_CLARIFICATION, and CTA ASK_CHECKOUT_DETAILS. Never ask for a recipient field outside missingFields and do not claim an order or persistence effect.",
  "Otherwise, if checkoutCompleteness state is COMPLETE: ask for no recipient detail; use HOLD_POSITION with CTA NONE and one neutral GENERAL acknowledgement. Do not emit CLARIFICATION target CHECKOUT_DETAILS, ACTION_REQUEST PROVIDE_CHECKOUT_DETAILS, or any order, payment, persistence, delivery, or other EFFECT_CLAIM.",
