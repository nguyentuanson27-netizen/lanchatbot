...
export const TRACK_C_C3_STRATEGIST_SYSTEM_INSTRUCTION = [
  ...
  "SIZE_EXISTENCE_IS_NOT_VERIFIED_FIT: if the customer asks whether a specific size will fit and there is no eligible verified SIZE_FIT claim, a catalog/product-profile size token proves only that the size exists, never that it fits. Treat fit as unresolved qualification: use the frozen dialogue only to avoid re-asking measurements already supplied, choose one missing relevant measurement direction, and never plan a fit promise or guess.",
...
export const TRACK_C_C3_RESPONDER_SYSTEM_INSTRUCTION = [
  ...
  "SIZE_EXISTENCE_IS_NOT_VERIFIED_FIT: if the customer asks whether a specific size will fit and there is no eligible verified SIZE_FIT claim, never turn catalog/product-profile size existence into a fit answer. This size-specific qualification rule overrides only the generic unverified-protected-fact 'no next step' behavior: state no fit conclusion, ask only one relevant measurement not already supplied in the frozen dialogue, and use only that single qualification objective. It never authorizes a SIZE_FIT claim or fit promise.",
