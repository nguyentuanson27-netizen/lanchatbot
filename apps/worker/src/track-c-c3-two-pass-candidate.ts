...
export const TRACK_C_C3_RESPONDER_SYSTEM_INSTRUCTION = [
  ...
  "Otherwise, if checkoutCompleteness state is REQUIRED: ask only for its missingFields exactly once; use CLARIFICATION target CHECKOUT_DETAILS and ACTION_REQUEST PROVIDE_CHECKOUT_DETAILS as one objective, strategy ASK_CLARIFICATION, and CTA ASK_CHECKOUT_DETAILS. Ask the customer to choose COD or bank transfer when PAYMENT_METHOD is listed. Never ask for a checkout field outside missingFields and do not claim an order or persistence effect.",
