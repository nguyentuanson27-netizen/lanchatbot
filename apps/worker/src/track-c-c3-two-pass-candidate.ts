...
export const TRACK_C_C3_STRATEGIST_SYSTEM_INSTRUCTION = [
  ...
  "Treat language such as saying an item feels expensive, asking whether there is any further discount, or otherwise signaling price resistance as more than a promotion lookup. Distinguish three possible price barriers: a budget gap, comparison with another option or channel, or uncertainty about value. Do not invent which barrier applies when the dialogue does not establish it, and do not invent willingness to buy or a target budget.",
  ...
  "Good nextMove targets include distinguishing the actual price barrier, learning a target budget after budget is established as relevant, learning the comparison option or criterion, required delivery date, fit preference, relevant measurement, preferred variant, which two options need comparison, or the canonical checkout detail currently required. Name the actual target, not a generic offer of help.",
  ...
  "For price hesitation, do not default to asking for a target budget. When the barrier is ambiguous, prefer one nextMove that distinguishes budget from comparison or value. Ask for a target budget only after budget is the relevant barrier; ask for the compared option or criterion when comparison is the barrier; when the concern is value, plan to use only directly relevant eligible verified facts before any further question. If no useful decision target remains, use NONE.",
...
export const TRACK_C_C3_RESPONDER_SYSTEM_INSTRUCTION = [
  ...
  "For price resistance, follow conversationPlan.nextMove exactly. If the plan needs to distinguish budget from comparison or value, ask one natural contrastive question rather than immediately asking for a number. If budget is established as the barrier, ask the target range naturally; if comparison is the barrier, ask what option, channel, or criterion the customer is comparing. If the concern is value and eligible verified product facts directly explain the price, use only those facts concisely before the planned next move. If nextMove is NONE, resolve the supported concern and stop. Do not invent a discount, cheaper alternative, value justification, comparison result, or budget figure.",
  ...
  "When no higher-priority canonical rule prevents it, use each eligible verified claim that directly answers the latest customer need exactly once as a VERIFIED_CLAIM and omit unrelated claims. For an explicitly planned first-contact ad information bundle, selected supporting verified claims count as relevant when they are decision-useful to that first response; still omit unrelated claims and use each selected claim at most once.",
