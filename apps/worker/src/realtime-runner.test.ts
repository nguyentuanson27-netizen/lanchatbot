...
describe("RealtimeRunner", () => {
  ...
  it("rehydrates SD375 before creating TEXT then three bounded full-look IMAGE units", async () => {
    ...
    const mediaRecognition: RealtimeMediaRecognitionPort = {
      recognize: vi.fn(async () => ({
        status: "MATCHED" as const, product: vectorProduct, score: 0.8167, gap: null,
        decisionSource: "GEMINI_RERANK" as const,
        candidates: [], reasonCode: "GEMINI_RERANK",
        telemetry: {
          ...
          embeddingDimension: 3_072,
          preprocessMaxDimension: 1_024,
          candidates: [],
          selectedEvidencePointId: "22222222-2222-5222-8222-222222222222",
          rerankerModel: "gemini-3.5-flash-lite",
          rerankerPromptVersion: "media-rerank-v2-cutout-only",
          rerankerDecision: "SD375",
          rerankerSelectedOriginalRank: 1,
          finalProductId: "SD375",
          finalRetrievalScore: 0.8167,
          errorCode: null,
          latencyMs: { prepare: 1, cutout: 1, embedding: 1, qdrant: 1, reranker: 1, total: 5 },
        },
      })),
    };
    const runner = new RealtimeRunner(
  ...
  it("keeps image order and sends only exact multi-product facts without deterministic CTA", async () => {
    ...
    const recognitionTelemetry = {
      ...
      errorCode: null,
      latencyMs: { prepare: 1, cutout: 1, embedding: 1, qdrant: 1, reranker: 0, total: 5 },
    };
    const cb182Candidate = {
      product: cb182,
      retrievalRank: 1,
      retrievalScore: 0.8,
      evidence: {
        pointId: "11111111-1111-5111-8111-111111111111",
        imageUrl: "https://lanadesign.vn/images/cb182-detail-01.jpg",
        imageRole: "ADDITIONAL",
        imageType: "DETAIL" as const,
        imageAngle: "CLOSEUP" as const,
        imageDetailType: "EMBROIDERY",
        imageContentSha256: null,
      },
    };
    const recognitions: readonly RealtimeMediaRecognition[] = [
      { status: "MATCHED", product: sv921, score: 0.9, gap: null, decisionSource: "GEMINI_RERANK", candidates: [], reasonCode: "GEMINI_RERANK", telemetry: recognitionTelemetry },
      {
        status: "AMBIGUOUS",
        candidates: [cb182Candidate],
        reasonCode: "GEMINI_AMBIGUOUS",
        telemetry: recognitionTelemetry,
      },
      { status: "MATCHED", product: cb182, score: 0.88, gap: null, decisionSource: "GEMINI_RERANK", candidates: [], reasonCode: "GEMINI_RERANK", telemetry: recognitionTelemetry },
    ];
