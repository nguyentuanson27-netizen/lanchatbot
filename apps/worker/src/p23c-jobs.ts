...
export function buildRegistryMap(rows: readonly Record<string, unknown>[]): RegistryMapResult {
  ...
  for (const [index, item] of rows.entries()) {
    ...
    registry[maSp] = {
      ...
      silhouette_override: tags(r.SILHOUETTE_OVERRIDE),
      occasion_override: tags(r.OCCASION_OVERRIDE),
      design_attributes_json: String(r.DESIGN_ATTRIBUTES_JSON || "").trim(),
      care_instructions: String(r.CARE_INSTRUCTIONS || "").trim(),
      wear_properties_json: String(r.WEAR_PROPERTIES_JSON || "").trim(),
      back_coverage: String(r.BACK_COVERAGE || "").trim().toUpperCase(),
      design_complexity: String(r.DESIGN_COMPLEXITY || "").trim().toUpperCase(),
...
export function buildApprovedQdrantJobs(
  ...
  for (const row of imageRegistryRows) {
    ...
    const payload: Record<string, unknown> = {
      ...
      description,
      description_authority: profile?.description_authority || "WEBSTORE_XML",
      description_source_version: profile?.description_source_version || profile?.source_hash || "catalog-v2",
      product_attributes: profile?.product_attributes || null,
      material,
