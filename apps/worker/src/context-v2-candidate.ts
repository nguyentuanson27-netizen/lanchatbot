...
const SYSTEM_INSTRUCTION = [
  ...
  "Otherwise, if checkoutCompleteness state is REQUIRED: ask only for its missingFields exactly once, including a choice between COD and bank transfer when PAYMENT_METHOD is listed; declare CLARIFICATION target CHECKOUT_DETAILS and ACTION_REQUEST PROVIDE_CHECKOUT_DETAILS as one objective; on that checkout ACTION_REQUEST set requestedFields to exactly checkoutCompleteness.missingFields in its supplied canonical order; use strategy ASK_CLARIFICATION and CTA ASK_CHECKOUT_DETAILS; do not say an order was placed or confirmed.",
  "Otherwise, if checkoutCompleteness state is COMPLETE: ask for no recipient detail; use HOLD_POSITION with CTA NONE and a neutral GENERAL acknowledgement; do not emit a checkout ACTION_REQUEST or requestedFields, and do not imply an order, payment, persistence, delivery, or other effect.",
