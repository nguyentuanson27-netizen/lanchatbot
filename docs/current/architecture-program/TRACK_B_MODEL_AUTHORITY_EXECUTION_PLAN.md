...
**Status:** `B2.4_MERGED / B3.1_V17_PREFLIGHT_FAILED_REMEDIATION_LOOP_AUTHORIZED / DIRECTION_SELECTED_POST_GENERATION_DEMOTION`
...
The concrete built operator is `node apps/worker/dist/track-b-commerce-authority-preprod-cli.js`. It accepts only fixed `ENGINEERING_PREPROD` page `1198992073286645` / `MESSENGER` inputs and reads the database credential only from the fixed secret file. `build` refuses a dirty or different checkout before and after the build, derives the build identity from the exact commit/tree plus an exact non-secret runtime-config projection, applies all three immutable image labels, and reads the image identity back. `prepare` verifies the exact running source and target image before binding three distinct startup identities into the packet: the unchanged source pointer for pre-CAS recovery, the operation target pointer, and the deterministic post-reverse-CAS pointer. Each path and canonical package hash is inside the self-hashed release-local operation packet. `execute`/`recover` reread all three files and verify their canonical hashes, exact pointer/service/release semantics and shared validated Track B evidence before constructing the database or service boundary. The bound source package must also receive `ADMITTED` from the same startup-authority implementation used by the realtime process, including historical/Track B evidence and rollback-transition rules. Start routing then requires the exact service plus pointer identity, so neither pre-CAS nor post-CAS recovery can select the other's startup package. The operator supports both fresh `V1 -> V2` activation/reactivation and a new-revision `V2 -> V1` rollback rehearsal; rollback must load the immutable activation rollback record by its exact self-hash, reuse that record's exact historical V1 authority/service identity, and create a fresh explicit Track B rollback startup package rather than replaying the old pointer revision. Runtime proof binds the exact pointer used for subsequent full-consumer proof; neither a CAS acknowledgment nor an unproved startup can populate that binding. `execute` and `recover` delegate every stage-container, service, fence, admission, pointer, runtime and recovery action to `TrackBCommerceAuthorityMutationPorts`; they cannot select another page, channel, compose project, container, secret path, or public-production environment.
...
16. Final owner decision records Track B completion and Track C start; source merge alone is not completion evidence.
...
Do not physically delete old implementations until a replacement exists, consumers are migrated, zero active use is proven, and rollback no longer depends on them.
