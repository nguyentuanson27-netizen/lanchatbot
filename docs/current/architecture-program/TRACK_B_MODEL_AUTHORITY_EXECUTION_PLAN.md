...
The concrete built operator is `node apps/worker/dist/track-b-commerce-authority-preprod-cli.js`. It accepts only fixed `ENGINEERING_PREPROD` page `1198992073286645` / `MESSENGER` inputs and reads the database credential only from the fixed secret file. `build` refuses a dirty or different checkout before and after the build, derives the build identity from the exact commit/tree plus an exact non-secret runtime-config projection, applies all three immutable image labels, and reads the image identity back. `prepare` verifies the exact running source and target image before creating immutable operation-target and reverse-CAS recovery startup packages, a release-local rollback record and a self-hashed direction-bound operation packet. Each recovery package pins the deterministic post-reversal pointer revision, so restarting the exact prior service cannot reuse a stale startup revision. The operator supports both fresh `V1 -> V2` activation/reactivation and a new-revision `V2 -> V1` rollback rehearsal; rollback must load the immutable activation rollback record by its exact self-hash, reuse that record's exact historical V1 authority/service identity, and create a fresh explicit Track B rollback startup package rather than replaying the old pointer revision. `execute` and `recover` delegate every stage-container, service, fence, admission, pointer, runtime and recovery action to `TrackBCommerceAuthorityMutationPorts`; they cannot select another page, channel, compose project, container, secret path, or public-production environment.
