...
Completion is recorded by `TRACK_B_PREPROD_ACCEPTANCE_20260905.md` under the
owner's `SOLO_PREPROD_MINIMAL` correction. The exact accepted V2 identity is
bound in `evidence/TRACK_B_INITIAL_LKG_V2_20260905.json`. No periodic or
arbitrarily newer runtime-resolution audit is required when the current pointer,
healthy service/image/config and mounted startup identity match exactly without
ambiguity. PR #321 is closed unmerged and is not part of the canonical path.
