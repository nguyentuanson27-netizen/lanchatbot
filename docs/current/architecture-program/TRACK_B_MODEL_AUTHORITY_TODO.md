...
- [x] Replay the immutable exact pre-B2.4 live capture versus the current live path on the same input and report differences without permitting claim/effect drift.
- [x] Pin model/provider-model identity, prompt/template identity, generation config, policy/schema/config identity and fixed fact fixtures.
- [x] Cover unsupported and protected claims, PII/security, unauthorized effects, stale facts, malformed output, the single-repair budget, verified-facts fallback and BF-04 size regressions.
