...
**Status:** `B2.4_MERGED / B3.1_V22_GATE_E_PASS / B3.2_SOURCE_PREPARATION_IN_PROGRESS / MIGRATION_0036_PREPROD_APPLIED / NO_DEPLOY_OR_POINTER_MUTATION`
...
- [x] Derive the truthful replacement bundle payload from source: strategy `AGENT_PROPOSAL_V1_STRATEGY_ANALYSIS`, CTA `AGENT_PROPOSAL_V1_SALES_SIGNALS`, unchanged fixed consumer set and no bypass classes. The canonical target bundle hash is `56b94f7a2e07e80fe8b2983a75b46caa78c2d48f3bd4081d4a88d8f40d2325b8`; the exact PREPROD rollback bundle remains `e423f3f647dce25cd74501555b73fc69cf66e4138fbfdda6b7e9c471fe89a05c` at pointer revision `6`, version `c88f3d7a-3c14-49ff-ab07-bcfbf664c643`. The target behavior version/content identity must still be prepared and read back from the database before activation.
...
- [ ] Merge the focused B3.2 source preparation: exact v22 registration/manifest/request/interpreter/Git-provenance re-derivation plus durable BODY/FINALIZATION readback, truthful V2 bundle, page-scoped writer, durable `0036` fence, quiescence/CAS/audit/full-consumer readback and exact V1 rollback path. The release packet also binds both exact pending `0037` artifacts and their separate-authorization status; it cannot represent them as applied. Before activation, persist the self-hashed release-local record binding exact previous and target `realtime-worker` release/build/image/config identities plus both authority identities; the mutation boundary must prove the recorded source and target service identities. The exact affected-service build is staged before the fence, but its service replacement occurs only after admission is held and zero authority-dependent work is proven; the same zero-work proof is repeated before CAS. Any replacement, pointer, release or readback ambiguity after replacement retains the fence. Rollback uses the same sequence with the recorded exact V1 service and authority identity. Migration `0036` is already separately authorized, applied and verified in `ENGINEERING_PREPROD`.
- [ ] Resolve the newly proven database dependency before activation: live migration `0036` guards only the first `LEGACY -> COMMERCE` cutover, so its insert trigger rejects the required exact `COMMERCE/V1 -> COMMERCE/V2` replacement and `V2 -> V1` rollback. Pending migration `0037_track_b_commerce_authority_replacement` extends only that guard for page `1198992073286645` / `MESSENGER` to the exact V1/V2 hashes while preserving the first cutover and fail-closing every other transition; source hashes are up `40b1ef14e3f7b2e037063de1f8d8ff7f804d069f8649115be6c29b1b56399c20`, down `070b2b793af7d6b6c33399531f47240cd9b5aa29d3dc12fe12ae0870acce57a7`. The owner authorized the corrected down hash after the first isolated rehearsal proved that the former composite-record/scalar `INTO` did not compile; a subsequent full restore proved that PostgreSQL dump/restore can flatten an equivalent nested Boolean `CHECK` expression and therefore cannot be compared safely by raw deparsed text alone. Raw live catalog identity remains separately pinned, while the authorized focused remediation compares every constraint name/schema/table/type/validation/deferrability plus a fail-closed canonical PostgreSQL `conbin` tree that only removes parser locations and flattens same-connective `AND`/`OR` nodes. Both failed rehearsals left live PREPROD unchanged and their disposable artifacts were removed. The down path additionally requires exact V1 version/bundle readback and zero live target-page fence before restoring the 0036 guard. It remains pending until the remediation is reviewed/merged and a complete fresh backup/rehearsal plus exact target readback pass.
