...
- [ ] Merge the replacement `TrackBCommerceAuthorityMutationPorts` PREPROD adapter and self-hashed operation packet. PR #310 is closed/superseded and excluded from the canonical path. The new record binds candidate V2 and LKG V2 source/tree, image/tag/build/config, pointer/version/bundle, startup hash, Gate E evidence and migrations-through-0038 compatibility. Only `ACTIVATE_V2_CANDIDATE` and `ROLLBACK_TO_LKG_V2` are admitted; missing/stale/ambiguous/incompatible LKG fails closed, and no V1 or third-authority fallback exists. The current V2 remains untouched in this source-only phase.
