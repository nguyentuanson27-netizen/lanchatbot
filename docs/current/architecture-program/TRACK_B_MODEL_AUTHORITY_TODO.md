...
**Status:** `B2.4_MERGED / B3.1_V21_REGISTRATION_PREPARED / MIGRATION_0036_PREPROD_APPLIED / NO_DEPLOY_OR_POINTER_MUTATION`
...
- [ ] Merge and anchor the immutable `evaluation/gate-e/track-b-v21` registration, then run exactly one governed bounded scored invocation. The registration artifacts are outside `GATE_E_CANDIDATE_SOURCE_PATHS_V1`, so the candidate remains bound to pre-registration main `8ffe7a0ded58fdf10b431717bb36200df2b8b912` without circular identity.
- [x] Use existing provenance and release-evidence machinery; do not invent a competing promotion manifest.
- [x] Keep provider credentials out of GitHub Actions and the repository; the v21 provider observation ran through the authorized PREPROD secret boundary and persisted redacted identity only.
