...
- [ ] Use the reviewed `deploy/track-b-0038-preprod-operator.sh` boundary for the separately owner-authorized exact-main fresh backup, isolated restore, real-PostgreSQL `up -> down -> up` acceptance, live apply and exact readback. The operator cannot deploy/start/stop services, create/release a fence, move the behavior pointer or send traffic; B3.2 activation remains a later distinct mutation after migration PASS.
