...
**Status:** `B2.4_MERGED / B3.1_V22_GATE_E_PASS / MIGRATIONS_0036_0037_0038_PREPROD_APPLIED_VERIFIED / B3.2_CONCRETE_ADAPTER_SOURCE_IN_PROGRESS / NO_DEPLOY_OR_POINTER_MUTATION`
...
- [ ] Merge the concrete `TrackBCommerceAuthorityMutationPorts` PREPROD adapter and self-hashed direction-bound operation-packet CLI. The source boundary pins page/channel, clean source commit/tree before and after build, compose project/container, both image tags+digests, the existing prior OCI revision plus a deterministic legacy build identity derived from its observed image and safe runtime-config hash, mandatory target OCI revision/build/config labels, an exact allowlisted runtime-config projection/readback, three distinct source/operation-target/reverse-CAS startup paths plus canonical content hashes, Gate E evidence, prior/target pointer identities and migrations-through-0038 admission proof; it uses only the reviewed database writers and exact Docker build/Compose service boundaries. All startup artifacts are reread and revalidated before any service/fence boundary, and the source package must pass the same realtime startup-authority admission logic. A stopped network-less stage container proves the target is materially staged without authority-dependent admission; partial/ambiguous staging enters exact cleanup instead of being reported as no mutation. The same executable boundary prepares/executes both fresh `V1 -> V2` activation/reactivation and new-revision `V2 -> V1` rollback, with an explicit Track B V1 rollback startup package. Real PostgreSQL readback plus concrete service-pair and orchestration failure-injection tests cover admission, drain, CAS/audit/runtime/consumer convergence and symmetric pre/post-CAS recovery. Source completion is not deploy or operational acceptance.
