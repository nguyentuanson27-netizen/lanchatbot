...
- [ ] Merge the concrete `TrackBCommerceAuthorityMutationPorts` PREPROD adapter and self-hashed operation-packet CLI. The source boundary pins page/channel, clean source commit/tree, compose project/container, both image tags+digests, the existing prior OCI revision plus a deterministic legacy build identity derived from its observed image and safe runtime-config hash, mandatory target OCI revision/build/config labels, an exact allowlisted runtime-config projection/readback, both startup packages, Gate E evidence, prior/target pointer identities and migrations-through-0038 admission proof; it uses only the reviewed database writers and exact Docker build/Compose service boundaries. A stopped network-less stage container proves the target is materially staged without authority-dependent admission. Real PostgreSQL readback plus orchestration failure-injection tests cover admission, drain, CAS/audit/runtime/consumer convergence and symmetric recovery. Source completion is not deploy or operational acceptance.
