...
15. State-advancing sales events (`APPLIED | HANDOFF`) form one contiguous stage/revision chain from the locked state to the proposed state. One shared predicate defines that set for the writer and database. Event labels do not authorize an arbitrary final `stage`, checkout draft, clarification, preview, confirmation, or negotiation snapshot.
...
| Non-open command changes `cart.expiresAt` while also changing `plan.cartExpiresAt` | Locked expiry wins and the transaction rejects `CART_EXPIRY_BINDING_MISMATCH` | database transaction tests |
...
| Readiness declares a lifetime longer than 60 seconds | Transaction rejects `EFFECT_READINESS_LIFETIME_INVALID` even when checked/expiry timestamps are otherwise fresh | database boundary test |
