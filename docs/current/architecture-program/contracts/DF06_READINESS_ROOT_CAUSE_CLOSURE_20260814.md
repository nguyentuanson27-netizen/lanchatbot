...
16. Every persisted sales-cycle plan must carry the literal `DF06_EFFECT_READINESS_V1` marker. A missing or mismatched marker is rejected before any event can bypass validation, and the locked complete top-level sales-state envelope is always validated. `schemaVersion`, `conversationKey`, `routing`, `processedCommandIds`, `revision`, `stage`, and `updatedAt` are derived from the locked state plus the contiguous applied-event chain; caller-controlled replacements are rejected.
...
23. The complete top-level key inventory is fail-closed for every persisted sales-cycle plan. An undeclared field cannot be persisted merely because existing validators do not know its semantics.
24. An exact negotiation-engine `REPLAYED` result is non-state-advancing at the sales boundary: it becomes `DUPLICATE`, keeps the state unchanged, and produces no committable event or plan. A persisted `NEGOTIATION_EVENT/APPLIED` always represents a new canonical transition to `CART_OPEN`; `ORDER_PREVIEW` cannot remain unchanged under that event label.
...
| Each legitimate `APPLIED` command kind | Its declared protected transition either replays exactly or preserves fields it does not own, and a real `store.commit` resolves | positive database transaction cases |
...
| Sales plan omits or changes the DF06 contract marker, including a non-commerce event | Transaction rejects `EFFECT_READINESS_CONTRACT_REQUIRED`; envelope validation is never opt-in | database typecheck and adversarial runtime test |
| Exact negotiation event/fingerprint replay | Runtime returns `DUPLICATE`, preserves revision and command ledger, and worker emits no event/plan; a forged `ORDER_PREVIEW` same-stage `APPLIED` event is rejected | contract Cartesian matrix, runtime replay test, and worker retry test |
