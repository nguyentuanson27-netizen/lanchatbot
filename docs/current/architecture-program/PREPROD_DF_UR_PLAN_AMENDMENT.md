...
The DF13 COMMERCE version schema and migration remain source-only until a
separate owner-authorized control-plane release. The current generic operator
creates only LEGACY versions and the generic CAS path rejects COMMERCE. A
dedicated, fence-bound adapter must wire the immutable `SalesAuthoritySnapshot`
to every authority consumer, audit/readback, and rollback before any future
cutover command is reviewed. This plan does not authorize that adapter or a
`LEGACY -> COMMERCE` runtime change.
