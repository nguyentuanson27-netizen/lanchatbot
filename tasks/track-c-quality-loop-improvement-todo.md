...
- [x] **1. Impact inventory + tests**
  ...
  - [x] Added focused coverage for V2 contract, exact `VERTEX_AI / global / gemini-3.6-flash` descriptor, `HIGH` request config, calibration removal, metrics preservation, and metrics/identity isolation.
...
- [x] **2. Slice A — judge 3.6 global / V2 / telemetry / review routing**
  - [x] Added narrow `judgeLocation` + `judgeModelName` support; generator paths remain on existing `location` / `modelName`.
  - [x] Bound Track C explicitly to `VERTEX_AI / global / gemini-3.6-flash` and fail closed before judge invocation on provider/location/model mismatch.
...
- [x] **4. Verification + review gate**
  ...
  - [ ] **Optional / not run:** provider-backed smoke for exact Vertex `global` + 3.6 + HIGH request. No provider-backed CI job or credential expansion was added.
...
- [x] Keep running all fifty quality fixtures after an individual fixture
  failure. Frozen PII-safe fixture replies are judged exactly and retained in
  owner-local history with their hashes; a provider failure retains the local
  pair when available plus a redacted `FAILED` code.
- [x] Retain exact frozen B3 accepted/candidate reply pairs with hashes in
  owner-local checkpoints/evidence for all seven cases; this does not expand
