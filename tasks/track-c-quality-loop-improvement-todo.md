...
- [x] **1. Impact inventory + tests**
  ...
  - [x] Added focused coverage for V2 contract, exact `VERTEX_AI / global / gemini-3.7-flash` descriptor, `HIGH` request config, calibration removal, metrics preservation, and metrics/identity isolation.
...
- [x] **2. Slice A — judge 3.7 global / V2 / telemetry / review routing**
  - [x] Added narrow `judgeLocation` + `judgeModelName` support; generator paths remain on existing `location` / `modelName`.
  - [x] Bound Track C explicitly to `VERTEX_AI / global / gemini-3.7-flash` and fail closed before judge invocation on provider/location/model mismatch.
...
- [x] **4. Verification + review gate**
  ...
  - [ ] **Optional / not run:** provider-backed smoke for exact Vertex `global` + 3.7 + HIGH request. No provider-backed CI job or credential expansion was added.
