...
const mutableKeys = new Set(['REALTIME_IMAGE', 'REALTIME_RELEASE_ID']);
...
const [container] = JSON.parse(execFileSync('docker', ['inspect', 'lana-chatbot-realtime-worker'], { encoding: 'utf8' }));
if (!container) throw new Error('BOUNDARY_CONTAINER_MISSING:realtime-worker');

const projection = {
  command: container.Config?.Cmd ?? null,
  entrypoint: container.Config?.Entrypoint ?? null,
  environmentSha256: digest(normalizedEntries(container.Config?.Env ?? [])),
...
const output = {
  schemaVersion: 1,
  infrastructureExceptRealtimePinsSha256: digest(normalizedEntries(infrastructure)),
  realtimeRuntimeBoundarySha256: digest(JSON.stringify(projection)),
...
console.log('realtime deployment boundary capture: PASS');
