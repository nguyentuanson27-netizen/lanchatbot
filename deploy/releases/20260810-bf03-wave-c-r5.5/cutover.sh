...
upsert_env_pin REALTIME_RELEASE_ID "$RELEASE_TAG"
...
test "$(container_env_value lana-chatbot-realtime-worker REALTIME_RELEASE_ID)" = "$RELEASE_TAG" || die "realtime release ID mismatch"
...
printf '%s\n' "CUTOVER_PASS tag=$RELEASE_TAG target=realtime-worker policy=OFF"
