...
docker build \
  ...
  --tag "$TARGET_REALTIME_IMAGE" \
  "$RELEASE_DIR"

install -d -m 0700 "$EVIDENCE_DIR"
docker image inspect --format '{{.Id}}' "$TARGET_REALTIME_IMAGE" > "$EVIDENCE_DIR/image-id"
chmod 600 "$EVIDENCE_DIR/image-id"
image_id="$(cat "$EVIDENCE_DIR/image-id")"
repo_digests="$(docker image inspect --format '{{json .RepoDigests}}' "$TARGET_REALTIME_IMAGE")"
rootfs_diff_ids="$(docker image inspect --format '{{json .RootFS.Layers}}' "$TARGET_REALTIME_IMAGE")"
revision_label="$(docker image inspect --format '{{index .Config.Labels "org.opencontainers.image.revision"}}' "$TARGET_REALTIME_IMAGE")"
release_label="$(docker image inspect --format '{{index .Config.Labels "lana.release.tag"}}' "$TARGET_REALTIME_IMAGE")"
docker image save "$TARGET_REALTIME_IMAGE" | gzip -n > "$EVIDENCE_DIR/image.tar.gz"
...
REALTIME_IMAGE="$TARGET_REALTIME_IMAGE" REALTIME_RELEASE_ID="$RELEASE_TAG" \
  docker compose --env-file "$INFRASTRUCTURE_ENV_FILE" -f "$COMPOSE_FILE" config --quiet
printf '%s\n' "BUILD_PASS tag=$RELEASE_TAG image=$TARGET_REALTIME_IMAGE"
