# Admin Policy Review r6.3 deployment

This directory is the reviewed production automation for
`20260809-admin-policy-review-r6.3`. It supersedes the unexecuted r6.2
automation and deploys the already merged Admin Policy Review stack to exactly
`admin-api` and `admin-web`. It does not run migrations, change routing, change
the page allowlist, mutate behavior modes, or send a Messenger test.
...
3. Run `run-build.sh`. It archives the immutable tag, builds the r6.3 image,
...
5. Run `cutover.sh`. It takes the global deployment lock, captures and verifies
   ...
   pins only the two target images, and runs
...
6. Cutover proves target image IDs, target health, every required and optional
   inventory identity, Admin config/secret-mount boundary parity, migration,
...
promotion must pass. It never deletes application data.
