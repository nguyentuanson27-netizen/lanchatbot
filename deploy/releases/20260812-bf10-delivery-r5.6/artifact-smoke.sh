...
test "$(docker image inspect --format '{{.Id}}' "$TARGET_DELIVERY_IMAGE")" = "$expected_id" || die "built image ID drift"
test "$(docker image inspect --format '{{index .Config.Labels "org.opencontainers.image.revision"}}' "$TARGET_DELIVERY_IMAGE")" = "$RELEASE_COMMIT" || die "OCI revision mismatch"
test "$(docker image inspect --format '{{index .Config.Labels "lana.release.tag"}}' "$TARGET_DELIVERY_IMAGE")" = "$RELEASE_TAG" || die "release label mismatch"
...
NODE
docker run --rm --network none --entrypoint node "$TARGET_DELIVERY_IMAGE" --check apps/worker/dist/delivery-server.js
docker run --rm --network none --entrypoint node "$TARGET_DELIVERY_IMAGE" -e '
  import("node:fs").then(({ accessSync }) => {
    for (const path of ["apps/worker/dist/delivery-server.js", "apps/worker/dist/meta-outbox-dispatcher.js", "packages/database/dist/realtime-runtime.js"]) accessSync(path);
  }).then(() => Promise.all([
    import("./apps/worker/dist/meta-outbox-dispatcher.js"),
    import("./packages/database/dist/realtime-runtime.js"),
  ])).catch((error) => { console.error(error); process.exit(1); });
'
printf '%s\n' "DELIVERY_ARTIFACT_SMOKE_PASS image=$TARGET_DELIVERY_IMAGE"
