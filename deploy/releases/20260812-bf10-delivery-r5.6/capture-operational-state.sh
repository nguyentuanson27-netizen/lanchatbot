...
cutover_at="${4:-1970-01-01T00:00:00Z}"
[[ "$cutover_at" =~ ^[0-9]{4}-[0-9]{2}-[0-9]{2}T[0-9]{2}:[0-9]{2}:[0-9]{2}Z$ ]] || die "invalid BF10 cutover timestamp"
...
closing="$(database_query "SELECT p.pointer_id, p.revision, v.version_id, v.version_number, v.revision, v.content_hash, COALESCE(v.content->>'replyReconciliationPolicy','OMITTED'), COALESCE(v.content->>'mediaPartialResolutionPolicy','OMITTED'), COALESCE(v.content->>'multiProductResolutionPolicy','OMITTED'), COALESCE(v.content->>'customerUrlPolicy','OMITTED'), (v.content ? 'correctionDialoguePolicy')::int FROM admin_active_pointers p JOIN admin_artifact_versions v ON v.version_id=p.version_id WHERE p.active AND p.page_id='$EXPECTED_PAGE_ID' AND p.channel='PUBLISHED' AND p.artifact_kind='CLOSING_STRATEGY'")"
...
bf10_aggregate="$(database_query "SELECT count(*), count(*) FILTER (WHERE last_error_code IS NOT NULL), count(*) FILTER (WHERE next_attempt_at IS NOT NULL), count(*) FILTER (WHERE attempt_count > 0), count(*) FILTER (WHERE accepted_at >= '$cutover_at'::timestamptz), count(*) FILTER (WHERE accepted_at >= '$cutover_at'::timestamptz AND last_error_code IS NOT NULL), count(*) FILTER (WHERE accepted_at >= '$cutover_at'::timestamptz AND next_attempt_at IS NOT NULL), count(*) FILTER (WHERE accepted_at >= '$cutover_at'::timestamptz AND attempt_count > 0) FROM meta_outbox WHERE status='SENT_ACCEPTED'")"
IFS='|' read -r bf10_all_accepted bf10_all_error bf10_all_retry bf10_all_attempt bf10_window_accepted bf10_window_error bf10_window_retry bf10_window_attempt <<<"$bf10_aggregate"
bf10_window_audit="$(database_query "SELECT count(*) FROM audit_log WHERE action='META_OUTBOX_ACCEPTED_AFTER_RETRY' AND created_at >= '$cutover_at'::timestamptz")"
...
node - "$output" \
  ...
  "$app_send_enabled" "$chatbot_send_enabled" "$realtime_mode" "$hourly_quota" "$daily_quota" \
  "$cutover_at" "$bf10_all_accepted" "$bf10_all_error" "$bf10_all_retry" "$bf10_all_attempt" \
  "$bf10_window_accepted" "$bf10_window_error" "$bf10_window_retry" "$bf10_window_attempt" "$bf10_window_audit" <<'NODE'
const [
  output,
  ...
  cutoverAt, allAccepted, allError, allRetry, allAttempt,
  windowAccepted, windowError, windowRetry, windowAttempt, windowAudit,
...
const state = {
  ...
  policy: {
    ...
    effective: { mediaPartialResolutionPolicy: mediaPolicy, multiProductResolutionPolicy: multiPolicy, customerUrlPolicy: urlPolicy },
  },
  behaviorMode: {
  ...
  bf10: {
    cutoverAt,
    historical: { accepted: integer(allAccepted), activeError: integer(allError), retryScheduled: integer(allRetry), attemptHistory: integer(allAttempt) },
    postCutover: { accepted: integer(windowAccepted), activeError: integer(windowError), retryScheduled: integer(windowRetry), attemptHistory: integer(windowAttempt), acceptedAfterRetryAudit: integer(windowAudit) },
    classification: cutoverAt === '1970-01-01T00:00:00Z'
      ? 'BASELINE_ONLY'
      : integer(windowAccepted) === 0
        ? 'PENDING_NATURAL_TRANSITION_EVIDENCE'
        : 'NATURAL_TRANSITION_WINDOW_OBSERVED',
  },
};
