...
readonly EXPECTED_DELIVERY_WORKER_ID="delivery-worker-1"
readonly EXPECTED_REALTIME_WORKER_ID="realtime-worker-1"
readonly EXPECTED_PREVIOUS_RELEASE="20260811-admin-policy-review-r6.11"
readonly EXPECTED_PREVIOUS_COMPOSE_SHA256="e59ab08b6ad42c2d1d2e3a5a11ce9e34a935921c866917b0d23b6c3c5d69ac33"
readonly EXPECTED_ROLLBACK_DELIVERY_IMAGE="lana-chatbot-app:realtime-compatibility-first-r32.2"
...
TARGET_DELIVERY_IMAGE="${TARGET_DELIVERY_IMAGE:-$EXPECTED_DELIVERY_IMAGE}"
RELEASE_SCRIPT_DIR="$(CDPATH= cd -- "$(dirname -- "${BASH_SOURCE[0]}")" && pwd)"
ROLLBACK_COMPOSE_OVERRIDE="$RELEASE_DIR/deploy/releases/$EXPECTED_RELEASE_TAG/delivery-rollback-image-override.yml"
...
require_release_provenance() {
  ...
  test "$TARGET_DELIVERY_IMAGE" = "$EXPECTED_DELIVERY_IMAGE" || die "unexpected target delivery image"
...
require_image_inputs() {
  : "${ROLLBACK_DELIVERY_IMAGE:?ROLLBACK_DELIVERY_IMAGE is required}"
  : "${ROLLBACK_DELIVERY_IMAGE_ID:?ROLLBACK_DELIVERY_IMAGE_ID is required}"
  : "${PREVIOUS_RELEASE_DIR:?PREVIOUS_RELEASE_DIR is required}"
  : "${PREVIOUS_COMPOSE_SHA256:?PREVIOUS_COMPOSE_SHA256 is required}"
  validate_image_ref "$ROLLBACK_DELIVERY_IMAGE"
  validate_image_id "$ROLLBACK_DELIVERY_IMAGE_ID"
  test "$ROLLBACK_DELIVERY_IMAGE" = "$EXPECTED_ROLLBACK_DELIVERY_IMAGE" || die "unexpected rollback delivery image"
  test "$ROLLBACK_DELIVERY_IMAGE_ID" = "$EXPECTED_ROLLBACK_DELIVERY_IMAGE_ID" || die "unexpected rollback delivery image ID"
...
rollback_override_source() {
  if test -d "$RELEASE_DIR"; then
    ...
    printf '%s\n' "$REPOSITORY_DIR/deploy/releases/$EXPECTED_RELEASE_TAG/delivery-rollback-image-override.yml"
  fi
}
...
rollback_compose() {
  ...
  ROLLBACK_DELIVERY_IMAGE="$ROLLBACK_DELIVERY_IMAGE" \
...
wait_healthy() {
  ...
  while (( $(date +%s) < deadline )); do
    status="$(docker inspect --format '{{if .State.Health}}{{.State.Health.Status}}{{else}}{{.State.Status}}{{end}}' lana-chatbot-delivery-worker 2>/dev/null || true)"
    test "$status" = "healthy" && return 0
    sleep 5
  done
  die "delivery-worker did not become healthy"
}
...
upsert_env_pin() {
  local key="$1" value="$2"
  case "$key" in
    DELIVERY_IMAGE) validate_image_ref "$value" ;;
...
save_non_target_ids() {
  ...
  while IFS=$'\t' read -r service container requirement; do
    test "$service" = "delivery-worker" && continue
