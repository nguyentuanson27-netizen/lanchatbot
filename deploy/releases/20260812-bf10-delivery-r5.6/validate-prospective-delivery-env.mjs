...
const prospectiveObject = compose.services?.['delivery-worker']?.environment;
if (!prospectiveObject || typeof prospectiveObject !== 'object' || Array.isArray(prospectiveObject) || !live || !image) {
  throw new Error('PROSPECTIVE_DELIVERY_ENV_INPUT_INVALID');
}
const allowed = new Set(allowedCsv.split(',').filter(Boolean));
const envMap = (entries, label) => {
  if (!Array.isArray(entries)) throw new Error(`PROSPECTIVE_DELIVERY_ENV_ARRAY_INVALID:${label}`);
  const result = new Map();
  for (const entry of entries) {
    if (typeof entry !== 'string' || !entry.includes('=')) throw new Error(`PROSPECTIVE_DELIVERY_ENV_ENTRY_INVALID:${label}`);
    const separator = entry.indexOf('=');
    const key = entry.slice(0, separator);
    if (!/^[A-Za-z_][A-Za-z0-9_]*$/u.test(key) || result.has(key)) {
      throw new Error(`PROSPECTIVE_DELIVERY_ENV_DUPLICATE_OR_KEY_INVALID:${label}:${key || 'EMPTY'}`);
    }
