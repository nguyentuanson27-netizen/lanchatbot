...
rollback_compose up -d --no-deps realtime-worker
wait_healthy

test "$(container_image_ref lana-chatbot-realtime-worker)" = "$ROLLBACK_REALTIME_IMAGE" || die "realtime rollback image mismatch"
test "$(container_image_id lana-chatbot-realtime-worker)" = "$ROLLBACK_REALTIME_IMAGE_ID" || die "realtime rollback image ID mismatch"
test "$(container_env_value lana-chatbot-realtime-worker REALTIME_RELEASE_ID)" = "$ROLLBACK_REALTIME_RELEASE_ID" || die "realtime rollback release ID mismatch"
test "$(docker inspect --format '{{.RestartCount}}' lana-chatbot-realtime-worker)" = "0" || die "realtime rollback restart count is not zero"
test "$(docker exec lana-chatbot-realtime-worker awk '/^Uid:/{print $2}' /proc/1/status)" = "1000" || die "rollback PID1 is not UID 1000"

# Restore source/runtime identity immediately after the exact r5.5 realtime target is back.
