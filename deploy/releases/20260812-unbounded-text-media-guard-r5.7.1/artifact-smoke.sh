...
docker run --rm --network none --entrypoint node "$TARGET_REALTIME_IMAGE" -e '
  import("node:fs").then(({ accessSync }) => {
    for (const path of ["apps/worker/dist/realtime-server.js", "apps/worker/dist/realtime-runner.js", "apps/worker/dist/bounded-concurrency.js", "apps/worker/dist/media-resolution.js", "apps/worker/dist/customer-url-policy.js", "apps/worker/dist/multi-product-clarification.js", "packages/contracts/dist/v3/business-fact-queries.js"]) accessSync(path);
  }).then(() => Promise.all([
    ...
    import("./apps/worker/dist/bounded-concurrency.js"),
    import("./packages/contracts/dist/v3/business-fact-queries.js"),
