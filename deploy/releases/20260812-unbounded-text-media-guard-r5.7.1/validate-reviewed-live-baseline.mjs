...
const closingFields = ['pointerId', 'pointerRevision', 'versionId', 'versionNumber', 'versionRevision', 'contentHash', 'replyPolicy', 'mediaPolicy', 'multiPolicy', 'urlPolicy', 'correctionPolicy'];
function parseClosingRow(raw) {
  const rows = parseDelimitedRows(raw, closingFields);
  if (rows.length !== 1) throw new Error('REVIEWED_BASELINE_CLOSING_ROW_COUNT');
  return closingFields.map((field) => rows[0][field]);
}
if (process.argv[2] === '--self-test') {
  ...
  const tabRow = expected.closing.join('\t');
  if (!isDeepStrictEqual(parseClosingRow(tabRow), expected.closing)) throw new Error('BASELINE_SELF_TEST_REJECTED:TAB_CLOSING_ROW');
  try { parseClosingRow(expected.closing.join('|')); throw new Error('BASELINE_SELF_TEST_ACCEPTED:PIPE_CLOSING_ROW'); } catch (error) { if (String(error).includes('BASELINE_SELF_TEST_ACCEPTED')) throw error; }
