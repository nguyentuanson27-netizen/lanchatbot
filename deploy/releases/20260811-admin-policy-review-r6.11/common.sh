...
readonly EXPECTED_RELEASE_TAG="20260811-admin-policy-review-r6.11"
readonly IMPLEMENTATION_COMMIT="116d46104113fc73a71108241414104b47e70e01"
readonly EXPECTED_ADMIN_IMAGE="lana-chatbot-app:admin-policy-review-r6.11"
readonly EXPECTED_MANIFEST_SHA256="cb1bec5022a0aa9e2f0ba328ba54446177a928744b1a87a31da2c4fd6ab1e9ab"
readonly EXPECTED_COMPOSE_SHA256="e59ab08b6ad42c2d1d2e3a5a11ce9e34a935921c866917b0d23b6c3c5d69ac33"
readonly EXPECTED_PREVIOUS_MIGRATION="0031_admin_policy_safe_deletion"
readonly EXPECTED_LATEST_MIGRATION="0031_admin_policy_safe_deletion"
readonly EXPECTED_MIGRATION_SHA256="e121786b814e0cb500be2970d2f350b2fae62f619b01c0af9fb7a8a2f1817047"
...
DATABASE_BACKUP_DIR="$APP_ROOT/backups/$RELEASE_TAG"
DATABASE_BACKUP_FILE="$DATABASE_BACKUP_DIR/lana_chatbot_pre_r611.dump"
DATABASE_BACKUP_SHA256_FILE="$DATABASE_BACKUP_FILE.sha256"
...
require_image_inputs() {
  ...
  : "${PRESERVED_ADMIN_API_IMAGE:?PRESERVED_ADMIN_API_IMAGE is required}"
  : "${PRESERVED_ADMIN_API_IMAGE_ID:?PRESERVED_ADMIN_API_IMAGE_ID is required}"
  ...
  validate_image_ref "$PRESERVED_ADMIN_API_IMAGE"
  validate_image_ref "$ROLLBACK_ADMIN_WEB_IMAGE"
  validate_image_id "$PRESERVED_ADMIN_API_IMAGE_ID"
...
database_migration_checksum() {
  ...
  [[ "$migration_name" =~ ^[0-9]{4}_[a-z0-9_]+$ ]] || die "invalid migration name"
  database_query "SELECT checksum_sha256 FROM schema_migrations WHERE migration_name = '$migration_name'"
}
