...
restore_database="lana_r611_restore_$$"
...
' sh "$restore_database")"
test "$restored_latest" = "$EXPECTED_LATEST_MIGRATION" || die "restored backup migration baseline mismatch"

test "$(docker exec "$postgres_container" sh -ceu '
  export PGPASSWORD="$POSTGRES_PASSWORD"
  exec psql -X -At -v ON_ERROR_STOP=1 -U "$POSTGRES_USER" -d "$1" -c \
    "SELECT checksum_sha256 || '"'"':'"'"' || (to_regclass('"'"'public.admin_artifact_deletions'"'"') IS NOT NULL)::int || '"'"':'"'"' || count(*) FROM schema_migrations, pg_trigger WHERE migration_name = '"'"'0031_admin_policy_safe_deletion'"'"' AND tgname IN ('"'"'admin_artifact_deletions_no_mutation'"'"','"'"'admin_active_pointers_deleted_version_guard'"'"') AND NOT tgisinternal GROUP BY checksum_sha256"
' sh "$restore_database")" = "$EXPECTED_MIGRATION_SHA256:1:2" || die "restored migration 0031 integrity verification failed"
