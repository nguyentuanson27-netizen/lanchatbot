...
test "$(container_image_ref lana-chatbot-admin-api)" = "$PRESERVED_ADMIN_API_IMAGE" || die "admin-api changed after preflight"
test "$(container_image_id lana-chatbot-admin-api)" = "$PRESERVED_ADMIN_API_IMAGE_ID" || die "admin-api image ID changed after preflight"
...
node "$script_dir/validate-target-evidence.mjs" \
  ...
  "$target_image_id" \
  "$PRESERVED_ADMIN_API_IMAGE" "$PRESERVED_ADMIN_API_IMAGE_ID" \
...
"$script_dir/backup-restore-test.sh"
...
"$script_dir/verify-production-schema.sh"
...
printf '%s\n' "CUTOVER_PASS tag=$RELEASE_TAG target=admin-web"
