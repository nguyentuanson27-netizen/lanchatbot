...
rollback_compose up -d --no-deps admin-web
wait_healthy admin-web

test "$(container_image_ref lana-chatbot-admin-api)" = "$PRESERVED_ADMIN_API_IMAGE" || die "admin-api changed during rollback"
test "$(container_image_id lana-chatbot-admin-api)" = "$PRESERVED_ADMIN_API_IMAGE_ID" || die "admin-api image ID changed during rollback"
