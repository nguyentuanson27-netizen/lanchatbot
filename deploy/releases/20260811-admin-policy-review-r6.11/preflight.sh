...
test "$(container_image_ref lana-chatbot-admin-api)" = "$PRESERVED_ADMIN_API_IMAGE" || die "admin-api image drift"
test "$(container_image_id lana-chatbot-admin-api)" = "$PRESERVED_ADMIN_API_IMAGE_ID" || die "admin-api image ID drift"
...
printf '%s\n' "PRECHECK_PASS tag=$RELEASE_TAG commit=$RELEASE_COMMIT target=admin-web"
