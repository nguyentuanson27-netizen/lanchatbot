...
try {
  ...
  const release = '20260810-admin-policy-review-r6.10';
  ...
  runtimeAfter.database = { latestMigration: '0031_admin_policy_safe_deletion', migrationLedgerDigest: bbbb };
  ...
  const runtimeArgs = [runtimeBeforePath, runtimeAfterPath, '0030_runtime_behavior_modes', '0031_admin_policy_safe_deletion', 'deployment'];
  if (run('validate-runtime-invariants.mjs', runtimeArgs).status !== 0) {
    throw new Error('valid migration plus target-only runtime transition rejected');
  }
  const noMigrationAfter = structuredClone(runtimeAfter);
  noMigrationAfter.database = structuredClone(runtimeBefore.database);
  writeFileSync(runtimeAfterPath, JSON.stringify(noMigrationAfter));
  if (run('validate-runtime-invariants.mjs', runtimeArgs).status === 0) {
    throw new Error('deployment accepted without migration 0031');
  }
  writeFileSync(runtimeAfterPath, JSON.stringify(runtimeAfter));
  runtimeAfter.routing.canaryPageIds = ['unexpected-page'];
  writeFileSync(runtimeAfterPath, JSON.stringify(runtimeAfter));
  const routingDrift = run('validate-runtime-invariants.mjs', runtimeArgs);
  ...
  writeFileSync(runtimeAfterPath, JSON.stringify(runtimeAfter));
  const rollbackMetadataDrift = run('validate-runtime-invariants.mjs', runtimeArgs);
  ...
  runtimeAfter.services.postgres.rollback.imageId = aaaa;
  writeFileSync(runtimeAfterPath, JSON.stringify(runtimeAfter));
  const rollbackArgs = [runtimeBeforePath, runtimeAfterPath, '0030_runtime_behavior_modes', '0031_admin_policy_safe_deletion', 'rollback'];
  if (run('validate-runtime-invariants.mjs', rollbackArgs).status !== 0) {
    throw new Error('rollback retaining additive migration 0031 was rejected');
  }
  runtimeAfter.database = structuredClone(runtimeBefore.database);
  writeFileSync(runtimeAfterPath, JSON.stringify(runtimeAfter));
  if (run('validate-runtime-invariants.mjs', rollbackArgs).status !== 0) {
    throw new Error('rollback after transactional migration failure was rejected');
  }
  ...
  requireText('common.sh', /EXPECTED_RELEASE_TAG="20260810-admin-policy-review-r6\.10"/, 'r6.10 identity is not pinned');
  requireText('common.sh', /EXPECTED_PREVIOUS_MIGRATION="0030_runtime_behavior_modes"/, 'previous migration identity is not pinned');
  requireText('common.sh', /EXPECTED_LATEST_MIGRATION="0031_admin_policy_safe_deletion"/, 'target migration identity is not pinned');
  requireText('common.sh', /EXPECTED_MIGRATION_SHA256="e121786b814e0cb500be2970d2f350b2fae62f619b01c0af9fb7a8a2f1817047"/, 'migration checksum is not pinned');
  ...
  requireText('backup-restore-test.sh', /pg_dump/, 'pre-migration database backup missing');
  requireText('backup-restore-test.sh', /pg_restore --exit-on-error/, 'database restore test missing');
  requireText('backup-restore-test.sh', /migration_down/, 'migration down compatibility test missing');
  requireText('migrate-production.sh', /docker run --rm --entrypoint cat "\$TARGET_ADMIN_IMAGE" "\$migration_in_image"/, 'production migration is not sourced from the exact target image');
  requireText('migrate-production.sh', /docker exec -i[\s\S]*"\$postgres_container" sh -ceu/, 'production migration does not use the database owner boundary');
  requireText('migrate-production.sh', /BEGIN;[\s\S]*INSERT INTO schema_migrations[\s\S]*COMMIT;/, 'production migration and ledger are not atomic');
  if (/admin_control_database_url|packages\/database\/dist\/migrate\.js up/.test(releaseScript('migrate-production.sh'))) {
    throw new Error('production DDL regressed to the restricted application role');
  }
  requireText('migrate-production.sh', /database_migration_checksum/, 'production migration ledger checksum gate missing');
  ...
  const migrateIndex = cutover.indexOf('migrate-production.sh');
  const recreateIndex = cutover.indexOf('compose up -d --no-deps admin-api admin-web');
  if (!(backupIndex >= 0 && backupIndex < armIndex && armIndex < migrateIndex && migrateIndex < recreateIndex)) {
    throw new Error('backup/migration/cutover ordering is unsafe');
  }
