# Admin Policy Review r6.10 deployment

This directory is the production automation for
`20260810-admin-policy-review-r6.10`. It supersedes the failed, automatically
rolled-back r6.9 cutover after production proved that the restricted Admin
application role cannot perform schema DDL. The rollback authority remains
r6.7. It
...
database, then reads the additive 0031 migration from the exact target image and
applies it as the PostgreSQL database owner, with the ledger update in the same
transaction, before the target services are recreated. It does not backfill or
rewrite data, change routing, change the
...
3. Run `run-build.sh`. It archives the immutable tag, builds the r6.10 image,
...
5. Run `cutover.sh`. It takes the global deployment lock, captures and verifies
   ...
   automatic rollback, pins only the two target images, reads migration 0031
   from the exact target image and applies it as the database owner in the same
   transaction as its ledger record, verifies the ledger and runs
